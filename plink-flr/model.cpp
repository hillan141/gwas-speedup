...
double old_buildAdditive(bool xchr, bool sex, bool i1, bool i2, bool haploid, double mAA, double mAB, double mBB, double mA, double mB, bool &skip) {
  if ( xchr ) 
    {
...
void Model::init_buildAdditive_cache() {
  for(int m = 0; m < 32; m++) {
    build_additive_skip_cache[m] = false;
    build_additive_cache[m] = old_buildAdditive(m & 1, m & 2, m & 4, m & 8, m & 16, mAA, mAB, mBB, mA, mB, build_additive_skip_cache[m]);
  }
}


inline int make_mask(bool i1, bool i2, bool i3, bool i4, bool i5) {
  return i1 | (i2 << 1) | (i3 << 2) | (i4 << 3) | (i5 << 4);
}

inline double Model::buildAdditive(Individual * person , int snp )
{
  // Additive effects (assuming individual-major mode)
  int s = additive[snp];  
  int m = make_mask(xchr[snp], person->sex, person->one[s], person->two[s], haploid[snp]);
  skip |= build_additive_skip_cache[m];
  return build_additive_cache[m];
}
...
void Model::buildDesignMatrix()
{
  ...
  init_buildAdditive_cache(); // precompute buildAdditive output.
  
  ///////////////////////////
  // Consider each individual

  X = vector< vector<double> >(P->n, vector<double>(np));
  int nb = 0;
  for (int i=0; i < P->n; i++)
    {
  ...
  if(need_checkVIF()) // do not check VIF for SSE optimized logistic model
    all_valid = checkVIF();
